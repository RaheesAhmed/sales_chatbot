...
.chat-options button {
      ...
      JSON.stringify([
        {
          sender: "bot",
          message:
            "Welcome! I'm Sales Agent ,your Helpful Assistant, here to assist you in making informed decisions. How can I help you today?",
        },
      ])
    );
  }
  ...
  function openGeneralChat() {
    ...
    appendMessage("bot", "Wellcome to Our Store. How can I assist you?");
      ...
      if (event.key === "Enter") {
        ...
        if (userInput !== "") {
          try {
            ...
            const response = await fetch(`${ServerAdd}/chat`, {
              method: "POST",
              headers: {
  ...
  async function sendMessage() {
    ...
    if (query !== "") {
      ...
      await sendChatInputToBackend(query);
    }
  }

  // Function to send open chat input to backend
  async function sendChatInputToBackend(userInput) {
    try {
      appendTypingAnimation();
